...
export const useScheduledPayments = () => {
  ...
  // Suscripción en tiempo real a cambios en pagos programados
  useEffect(() => {
    if (!user) return;

    const channel = supabase
      .channel(`rt-scheduled-payments-${user.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'scheduled_payments', filter: `user_id=eq.${user.id}` },
        () => {
          fetchScheduledPayments();
        }
      )
