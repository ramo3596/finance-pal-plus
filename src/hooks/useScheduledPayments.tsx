...
export const useScheduledPayments = () => {
  ...
  const confirmPayment = async (scheduledPaymentId: string, occurrenceDate: string) => {
    if (!user) return;

    try {
      const { data, error } = await supabase
        .from('scheduled_payment_confirmations')
        .upsert({
      ...
      await fetchScheduledPayments(true);
      return data;
    } catch (error) {
  ...
  const postponePayment = async (scheduledPaymentId: string, occurrenceDate: string) => {
    if (!user) return;
    ...
    try {
      ...
      toast({
        title: "Éxito",
        description: "Pago pospuesto correctamente",
      });
      ...
      toast({
        title: "Error",
        description: "No se pudo posponer el pago",
        variant: "destructive",
      });
      throw error;
    }
  ...
  const rejectPayment = async (scheduledPaymentId: string, occurrenceDate: string) => {
    if (!user) return;
  ...
  const getPaymentConfirmations = async (scheduledPaymentId: string) => {
    if (!user) return [];
  ...
  useEffect(() => {
    ...
    // Set up real-time subscription for scheduled_payment_confirmations
    if (user) {
      const channel = supabase
        .channel('scheduled_payment_confirmations_changes')
        .on(
          'postgres_changes',
          {
            event: '*',
            schema: 'public',
            table: 'scheduled_payment_confirmations',
            filter: `user_id=eq.${user.id}`,
          },
          (payload) => {
            console.log('Scheduled payment confirmation change received:', payload);
