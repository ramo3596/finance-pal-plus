...
interface TransferScheduledFormProps {
  onClose: () => void;
}
...
export const TransferScheduledForm = ({ onClose }: TransferScheduledFormProps) => {
  ...
  const watchFromAccount = form.watch('account_id');

  // Filter destination accounts to exclude the selected source account
  const availableToAccounts = accounts.filter(account => account.id !== watchFromAccount);
